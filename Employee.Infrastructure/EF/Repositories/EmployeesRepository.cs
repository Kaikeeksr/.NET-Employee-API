...
public class EmployeesRepository : IEmployeesRepository
{
    ...
    public EmployeesRepository(CleverCloudDbContext context, IConfigurationProvider mapper)
    {
        _context = context;
        _mapper = mapper;
    }
    ...
    public async Task<List<EmployeeResponse.GetEmployeeResponse>> GetAllAsync()
    {
        return await _context.TblEmployees
            .Where(e => e.EStatus != "Z")
            .OrderBy(e => e.EName)
            .ProjectTo<EmployeeResponse.GetEmployeeResponse>(_mapper)
            .AsNoTracking()
            .ToListAsync();
    }
...
*/
    ...
    public async Task<EmployeeResponse.CreateEmployeeResponse?> CreateEmployeeAsync(EmployeeRequest.CreateEmployeeRequest employee)
    {
        try
        {
            ...
            var employeeEntity = mapper.Map<TblEmployees>(employee);

            await _context.TblEmployees.AddAsync(employeeEntity);
            await _context.SaveChangesAsync();

            return mapper.Map<EmployeeResponse.CreateEmployeeResponse>(employeeEntity);
        }
    ...
    EmployeeRequest.UpdateEmployeeRequest request)
    {
        try
        {
            ...
            return mapper.Map<EmployeeResponse.UpdateEmployeeResponse>(entity);
        }
        catch (DbUpdateException ex)
        {
